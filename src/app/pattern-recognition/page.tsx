...
function StepCard({
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {revealed ? (
            ...
            <span
              style={{ fontSize: 14, fontWeight: 800, color: 'var(--tx-3)' }}
            >
              {stepNum}
            </span>
          )}
        ...
        <div style={{ flex: 1 }}>
          <div
            style={{
              fontSize: 14,
              fontWeight: 700,
              color: revealed ? color : 'var(--tx-2)',
            }}
          >
            {title}
          </div>
          <div style={{ fontSize: 12, color: 'var(--tx-4)', marginTop: 2 }}>
            {subtitle}
          </div>
        </div>
        {!revealed && (
      ...
      {revealed && (
        <div
          style={{ padding: '0 16px 16px', borderTop: `1px solid ${color}25` }}
        >
          <div style={{ height: 10 }} />
          {children}
        </div>
...
> = {
  'sliding-window': {
    ...
    n_kyu:
      'Kyunki n=10⁶ mein O(n²) = 10¹² operations = TLE. O(n) = 10⁶ operations = AC ✅',
    time_needed: 'O(n)',
    input_hint: 'Array ya String diya hai',
  ...
  'two-pointers': {
    ...
    n_kyu:
      'Sorted array mein O(n²) brute force slow hai. Two Pointers = O(n) mein pair dhundho ✅',
    time_needed: 'O(n)',
    input_hint: 'Sorted array ya string',
  ...
  'binary-search': {
    ...
    n_kyu:
      'n=10⁹ mein O(n) = 10⁹ operations = TLE. O(log n) = sirf 30 operations = AC ✅',
    time_needed: 'O(log n)',
    input_hint: 'Sorted array ya monotonic condition',
  ...
  dfs: {
    ...
    n_kyu:
      'Tree/graph mein har node ek baar visit karo = O(V+E). Recursion stack = O(h) space ✅',
    time_needed: 'O(V+E) ya O(m×n)',
    input_hint: 'Tree ya 2D grid',
  ...
  bfs: {
    ...
    n_kyu:
      'BFS queue mein har node sirf ek baar aata hai = O(V+E). Level = shortest distance ✅',
    time_needed: 'O(V+E)',
    input_hint: 'Graph ya grid',
  ...
  'dynamic-programming': {
    ...
    n_kyu:
      'Overlapping subproblems mein recursion same cheez baar baar solve karta hai. DP memoize karta hai = O(n²) ✅',
    time_needed: 'O(n²) ya O(n)',
    input_hint: 'Array ya string',
  ...
  heap: {
    ...
    n_kyu:
      'Sort karna O(n log n) — zyada slow. Heap size k maintain karo = O(n log k) — k << n to bahut fast ✅',
    time_needed: 'O(n log k)',
    input_hint: 'Unsorted array ya stream',
  ...
  'topological-sort': {
    ...
    n_kyu:
      'Dependencies ki ordering mein har node/edge ek baar process hoti hai = O(V+E) ✅',
    time_needed: 'O(V+E)',
    input_hint: 'Directed graph ya dependencies list',
  ...
  graph: {
    ...
    n_kyu:
      'Union-Find mein path compression + rank se har operation almost O(1) hota hai ✅',
    time_needed: 'O(n α(n)) ≈ O(n)',
    input_hint: 'Graph edges ya grid',
  ...
  subsets: {
    ...
    n_kyu:
      'n ≤ 20 mein 2^20 = ~1M operations OK hai. n=40 hota to TLE. Small n = backtracking signal ✅',
    time_needed: 'O(2^n ya n!)',
    input_hint: 'Small array (n ≤ 20)',
  ...
  'merge-intervals': {
    ...
    n_kyu:
      'Sort + single pass = O(n log n). Unsorted mein O(n²) comparison lagti. Sort karo ek baar, phir O(n) ✅',
    time_needed: 'O(n log n)',
    input_hint: 'Array of intervals [start, end]',
  ...
  'cyclic-sort': {
    ...
    n_kyu:
      'Har number sirf ek baar swap hota hai (apni correct position pe jaata hai) = O(n) total ✅',
    time_needed: 'O(n)',
    input_hint: '1 to n range ke numbers',
  ...
  'fast-slow-pointers': {
    ...
    n_kyu:
      'Slow aur fast pointer: fast 2x speed se jaata hai, cycle mein hamesha milenge = O(n), O(1) space ✅',
    time_needed: 'O(n)',
    input_hint: 'Linked list',
  ...
  'bit-manipulation': {
    ...
    n_kyu:
      'Bit operations CPU level pe hote hain — O(1) per operation. XOR: a^a=0 se duplicates cancel ✅',
    time_needed: 'O(n)',
    input_hint: 'Integer array',
  ...
  trie: {
    ...
    n_kyu:
      'HashMap pe prefix check O(n) hogi. Trie mein prefix path follow karo = O(L) where L=word length ✅',
    time_needed: 'O(L) per query',
    input_hint: 'Array of strings',
...
function TrainerTab() {
  ...
  const detailQ = RECOGNITION_CHALLENGES.find(
    (r) => r.title === stub?.title
  ) as RecognitionChallenge | undefined;
  ...
  const options = useMemo(() => {
    ...
    const otherIds = Object.keys(PATTERN_META_MAP).filter(
      (p) => p !== correctId
    );
  ...
  const revealStep = (n: number) =>
    setRevealedSteps((prev) => new Set([...prev, n]));
  const next = () => {
    setQIdx((i) => i + 1);
  ...
  const handleChoose = (p: string) => {
    ...
    setChosen(p);
    setScore((s) => ({
      correct: s.correct + (p === stub?.patternId ? 1 : 0),
      total: s.total + 1,
    }));
  };
  ...
  if (!stub) return null;
  const lcUrl =
    stub.leetcodeUrl || `https://leetcode.com/problems/${stub.slug}/`;
  ...
  const step1_n =
    detailQ?.step1.n_value ?? patternHint?.n_hint ?? 'Constraints dekho';
  const step1_kyu =
    patternHint?.n_kyu ??
    'Bade n ke liye algorithm ki efficiency important hoti hai';
  const step1_time =
    detailQ?.step1.time_needed ?? patternHint?.time_needed ?? '';
  const step2_input =
    detailQ?.step2.input_type ??
    patternHint?.input_hint ??
    'Input type check karo';
  const step2_hints = detailQ?.step2.hints ?? [];
  const step3_output =
    detailQ?.step3.output_type ??
    patternHint?.output_hint ??
    'Output type check karo';
  const step3_hints = detailQ?.step3.hints ?? [];
  const keywords =
    detailQ?.step4_keywords ?? patternHint?.keywords ?? stub.tags ?? [];
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <option value="all">
              All Questions ({ALL_QUESTION_STUBS.length})
            </option>
            {Object.entries(PATTERN_META_MAP).map(([k, v]) => (
              <option key={k} value={k}>
                {v.icon} {v.name} (
                {ALL_QUESTION_STUBS.filter((q) => q.patternId === k).length})
              </option>
            ))}
          </select>
          <span
            style={{ fontSize: 12, color: 'var(--tx-4)', whiteSpace: 'nowrap' }}
          >
          ...
          {score.total > 0 && (
            <span
              style={{
                ...
                fontWeight: 700,
                color:
                  score.correct / score.total >= 0.7 ? '#10b981' : '#ef4444',
              }}
            >
        ...
        >
          ...
          >
            <span
              style={{ fontSize: 16, fontWeight: 800, color: 'var(--tx-1)' }}
            >
              #{stub.id}. {stub.title}
            </span>
            <a
          ...
          >
            <span
              style={{
                ...
                padding: '3px 10px',
                borderRadius: 12,
                background: `${DIFF_C[stub.difficulty] ?? '#10b981'}20`,
                color: DIFF_C[stub.difficulty] ?? '#10b981',
                border: `1px solid ${DIFF_C[stub.difficulty] ?? '#10b981'}40`,
              }}
            >
              {stub.difficulty}
            </span>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
          ...
          >
            📖 <strong>Pehle LeetCode pe problem padho</strong> — examples
            dekho, constraints samjho. Phir wapas aao aur right side ke steps
            follow karke pattern identify karo.
            <br />
            <a
              href={lcUrl}
              target="_blank"
      ...
      >
        ...
        >
            ...
            >
              <div
                style={{
                  ...
                  background: revealedSteps.has(s)
                    ? 'var(--accent)'
                    : 'var(--bg-elevated)',
                  display: 'flex',
                  alignItems: 'center',
              ...
              {i < 3 && (
                <div
                  style={{
                    flex: 1,
                    height: 2,
                    background: revealedSteps.has(s + 1)
                      ? 'var(--accent)'
                      : 'var(--border)',
                    margin: '0 6px',
                    transition: 'all 0.3s',
          ...
          >
            {revealedSteps.size === 0
              ? 'Step 1 se shuru karo 👆'
              : `${revealedSteps.size}/4 dekhe`}
          </span>
        </div>
        ...
        >
          ...
          <StepCard
            stepNum={1}
            revealed={revealedSteps.has(1)}
            onReveal={() => revealStep(1)}
            title="Constraints dekho — kitna time milega?"
            subtitle="n ki value → allowed time complexity decide hoti hai"
            icon="🔢"
            color="#6366f1"
          >
            <div
            ...
            {step1_time && (
              ...
              >
                ✅ Time complexity chahiye:{' '}
                <strong style={{ color: '#6366f1' }}>{step1_time}</strong>
              </div>
            )}
          ...
          <StepCard
            stepNum={2}
            revealed={revealedSteps.has(2)}
            onReveal={() => revealStep(2)}
            title="Input dekho — kaunsa pattern fit hoga?"
            subtitle="Input ka type → pattern narrow ho jaata hai"
            icon="📥"
            color="#f59e0b"
          >
            <div
            ...
            {step2_hints.length > 0 ? (
              ...
              >
                💡 Ye input type dekh ke socho: kaunse patterns is input ke
          ...
          <StepCard
            stepNum={3}
            revealed={revealedSteps.has(3)}
            onReveal={() => revealStep(3)}
            title="Output dekho — kya return karna hai?"
            subtitle="Output ka type → approach decide hoti hai"
            icon="📤"
            color="#10b981"
          >
            <div
            ...
            {keywords.length > 0 && (
              ...
              >
                ...
                >
                  Ye keywords dikhein to seedha pattern pehchaan lo:
                </div>
          ...
          <StepCard
            stepNum={4}
            revealed={revealedSteps.has(4)}
            onReveal={() => revealStep(4)}
            title="Pattern + Algorithm — final decision!"
            subtitle="Kaunsa type, kaunsa DS, aur kyu?"
            icon="🎯"
            color="#ec4899"
          >
            <div
            ...
            {detailQ ? (
              <>
                <div
                  style={{
                    ...
                    marginBottom: 12,
                    padding: '12px 16px',
                    background: `${correctMeta?.color}10`,
                    borderRadius: 10,
                    border: `1px solid ${correctMeta?.color}30`,
                  }}
                >
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700 }}>
                      <span style={{ color: correctMeta?.color }}>
                        T: {detailQ.time_complexity}
                      </span>
                      <span style={{ color: 'var(--tx-4)', margin: '0 5px' }}>
                        |
                      </span>
                      <span style={{ color: 'var(--tx-2)' }}>
                        S: {detailQ.space_complexity}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <br />
                Is pattern ke saath practice karo →{' '}
                <Link
                  href={`/patterns/${stub.patternId}`}
                  style={{ color: correctMeta?.color, fontWeight: 700 }}
                >
                  ...
                  return (
                    <button
                      ...
                      onMouseEnter={(e) => {
                        ...
                        el.style.border = `1px solid ${m?.color}60`;
                ...
                >
                  ...
                  <div>
                    ...
                    {!isCorrect && (
                      ...
                      >
                        Sahi: {correctMeta?.icon}{' '}
                        <strong>{correctMeta?.name}</strong>
                      </div>
                    )}
                  </div>
                </div>
...
function DeepGuideTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (selPattern !== 'all')
      rules = rules.filter((r) => r.pattern === selPattern);
    if (search.trim()) {
  ...
  const patternOptions = [
    ...new Set(DECISION_RULES.map((r: DecisionRule) => r.pattern)),
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          📊 Deep Decision Guide — {DECISION_RULES.length} Rules
        </div>
        <div style={{ fontSize: 13, color: 'var(--tx-2)' }}>
          Har rule: kab lagao, kyu lagao, kya galti mat karo — Hinglish mein
        </div>
      </div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        ...
        <select
          value={selPattern}
          onChange={(e) => setSelPattern(e.target.value)}
          style={{
            padding: '7px 12px',
            borderRadius: 8,
            border: '1px solid var(--border)',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {filtered.map((rule: DecisionRule) => {
          const meta = PATTERN_META_MAP[rule.pattern] ?? {
            color: '#10b981',
            icon: '🔹',
            name: rule.pattern,
          };
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {meta.name} → {rule.pattern_type}
                  </div>
                </div>
                {isOpen ? (
                  <ChevronUp size={16} color="var(--tx-3)" />
                ) : (
                  <ChevronDown size={16} color="var(--tx-3)" />
                )}
              </button>
              {isOpen && (
                <div
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: 'var(--tx-2)' }}>
          Har pattern ke types — triggers dekho → DS choose karo → approach
          samjho → question solve karo
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {ALL_PATTERN_TYPES_DATA.map((pat) => {
          const meta = PATTERN_META_MAP[pat.id] ?? {
            color: pat.color ?? 'var(--accent)',
            icon: pat.icon ?? '🔹',
            name: pat.name,
          };
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {types.length} types • Click karo → details dekho
                  </div>
                </div>
                {isOpen ? (
                  <ChevronUp size={16} color="var(--tx-3)" />
                ...
                >
                  ...
                  >
                    {types.length === 0 ? (
                      ...
                      >
                        Ye pattern ka alag type nahi hota — seedha use karo.
                      </div>
                    ) : (
                      types.map((sub, si) => (
                        <div
                        ...
                        >
                          ...
                          {sub.triggers?.length > 0 && (
                            <div style={{ marginBottom: 8 }}>
                              ...
                              <span
                                style={{ fontSize: 12, color: 'var(--tx-2)' }}
                              >
                                {Array.isArray(sub.triggers)
                                  ? sub.triggers.join(' | ')
                                  : sub.triggers}
                              </span>
                            </div>
                          ...
                          >
                            Time: {sub.complexity?.time} | Space:{' '}
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div
              style={{ fontSize: 18, fontWeight: 800, color: 'var(--tx-1)' }}
            >
              Pattern Recognition Training
            </div>
            <div style={{ fontSize: 12, color: 'var(--tx-3)' }}>
              {ALL_QUESTION_STUBS.length} LeetCode problems ·{' '}
              {DECISION_RULES.length} decision rules · Hinglish mein
            </div>
          </div>
